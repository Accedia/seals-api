...
def extractPdfFile(file_name, pages):
    tables = camelot.read_pdf(file_name, pages=pages)
    res = []
    for t in tables:
        res.append(t.df.values)
    return res
...
def processTable(dataArray):
    res = []
    for i in range(1, len(dataArray)):
        measurement = processMeasurement(dataArray[i])
...
# rawDataVarna = extractPdfFile(VARNA_FILE_NAME, "1-2") # "1-end"

# for d in rawDataVarna:
#     table = processTable(d)
...
collection = db.get_collection('beaches')
books_to_update = collection.insert_many(burgasBeaches)
